...
function start() {
	...
	if (document.documentMode || /Edge/.test(navigator.userAgent)) {
		...
		document
			.querySelector('#nearestLocation')
			.addEventListener('click', () =>
				toastr['info'](
					'Denne funksjoner er ikke tilgjenlig hos nettleserene <strong>Internet Explorer</strong> eller <strong>Microsoft Edge</strong>'
				)
			);
	} else {
	...
	document
		.querySelector('#toForm')
		.addEventListener('click', () => document.querySelector('#sendShelterBtn').click());
}
...
function openMainMap() {
	...
	document
		.querySelector('#openMainMap')
		.querySelector('#closeModal')
		.addEventListener('click', () => (document.body.style.overflowY = 'scroll'));
...
function fillMainMap(shelters) {
	...
	shelters.features.forEach((shelter) => {
        ...
        <div class="mapInfoCard">
            ...
            <ul class="mapList">
                ${Object.keys(shelter.properties)
					.map(
						(key) => '<li><strong>' + key.toUpperCase() + '</strong>: ' + shelter.properties[key] + '</li>'
					)
					.join(' ')}
            </ul>
        </div>
		...
		setTimeout(function() {
			...
			if (isClosest) {
				...
				document.querySelector('#resultFor').innerHTML =
					"Nærmeste tilfluktsrom fra deg er <br><span class='mt-5 h4-responsive font-weight-bold text-center'>" +
					info.adresse +
					'</span>';
			} else {
...
function loading(noFound) {
	...
	setTimeout(function() {
		if (seeAllShelters) {
			document.querySelector('#resultFor').innerHTML =
				"Viser alle <span class='mt-5 h4-responsive font-weight-bold text-center'>registrerte</span> tilfluktsrom";
			return;
		}

		if (!isClosest) {
			document.querySelector('#resultFor').innerHTML =
				"Søketreff for <span class='mt-5 h4-responsive font-weight-bold text-center'>" +
...
function createMap(ele, lat, lng, expand) {
	...
	if (expand) {
		// streetview query
		const url =
			'https://maps.googleapis.com/maps/api/streetview?size=1000x1000&location=' +
			lat +
			',' +
			lng +
			'&heading=90&pitch=-0.76&key=AIzaSyA_jiuMdTONboV9E0sHZ5U5-js9zwyd4GU';
	...
	setTimeout(function() {
		...
		shelters.features.forEach((shelter) => {
			...
			if (info.adresse.toLowerCase() === searchValue) {
				...
				info.distriktsnavn.toLowerCase().includes(searchValue) ||
				info.distriktsnavn.toLowerCase() === searchValue
			) {
...
function paginateNavigate() {
	document
		...
		.forEach((ele) => (ele.className = 'page-item pr-1 pl-1 paginationItem'));
